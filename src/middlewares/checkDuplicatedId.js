...
const { BadRequestError, ConflictError, InternalSercerError } = require("../model/customError");
const pool = require("../../database/connect/postgre");

const checkDuplicatedId = async (req, res, next) => {
    const { id } = req.body;
    try {
        const selectLoginQueryResult = await pool.query('SELECT 1 FROM calenduck.login WHERE id = $1', [id]);
